...
const getDireccion = async (req, res) => {
	const response = await con.query("SELECT * FROM public.direcciones ORDER BY id_direccion ASC");
	res.status(200).json(response.rows);
};
...
const postDireccion = async (req, res) => {
	var params = req.body;
	const query = {
		text: "INSERT INTO public.direcciones (detalle, sigla) VALUES ($1, $2)",
		values: [params.detalle, params.sigla]
	}
...
const postDirecciones = async (req, res) => {
	var params = req.body;
	const query = {
		text: `select * from public.direcciones where id_direccion=$1`,
		values: [
			params.id_direccion,
		],
	};
...
const putDirecciones = async (req, res) => {
	...
	var params = req.body;
	const query = {
		text: `UPDATE public.direcciones SET detalle=$1, sigla=$2 WHERE id_direccion=${id}`,
		values: [
			params.detalle,
            params.sigla,
		],
